...
export default function KnightsTour() {
  ...
  const [gameMode, setGameMode] = useState("quick"); // "quick" or "trace"
  ...
  useEffect(() => {
    if (vizPlaying) {
      vizIntervalRef.current = setInterval(() => {
        setVizStep(prev => {
          if (prev >= solution.length - 1) { setVizPlaying(false); return prev; }
          return prev + 1;
        });
      }, vizSpeed);
    }
  ...
  // Get valid next moves from current position
  const getValidMoves = (row, col) => {
    ...
    return moves
      ...
      .filter(pos =>
        ...
        !tracedPath.some(p => p.row === pos.row && p.col === pos.col)
      );
  };

  const handleCellClick = async (row, col) => {
    if (gameMode !== "trace" || gameState !== "playing") return;

    // First click — must be start position
    if (tracedPath.length === 0) {
      if (row === startPos.row && col === startPos.col) {
        setTracedPath([{ row, col }]);
      } else {
        alert("Start from the knight's position! (highlighted in blue)");
      }
      return;
    }

    const lastPos = tracedPath[tracedPath.length - 1];

    // Check already visited
    if (tracedPath.some(p => p.row === row && p.col === col)) {
      alert("You already visited this square!");
      return;
    }

    // Check valid knight move
    if (!isValidKnightMove(lastPos, { row, col })) {
      setWrongMove({ row, col });
      setTimeout(() => setWrongMove(null), 600);
      alert("Invalid knight move! A knight moves in an L-shape.");
      return;
    }

    const newPath = [...tracedPath, { row, col }];
    setTracedPath(newPath);

    // Check win — visited all squares
      ...
      try {
        const res = await fetch(`${API_URL}/api/knights-tour/submit`, {
          ...
          body: JSON.stringify({
            ...
            playerAnswer: String(boardSize * boardSize),
    ...
    // Check stuck — no valid moves left
    const validMoves = getValidMoves(row, col);
    if (validMoves.length === 0 && newPath.length < boardSize * boardSize) {
      setResult("lose");
      setCorrectAnswer(boardSize * boardSize);
      setGameState("result");
    }
  };
  ...
  const fetchLeaderboard = async () => {
    ...
    } catch { alert("Could not fetch leaderboard!"); }
  ...
  const fetchDashboard = async () => {
    ...
    } catch { alert("Could not fetch dashboard data!"); }
    setDashboardLoading(false);
  };

  const openVisualization = () => { setVizStep(0); setVizPlaying(false); setShowVisualization(true); };
  ...
  const resultConfig = {
    win:  { emoji: "🏆", color: "#4ade80", title: "Correct! Well done!", msg: gameMode === "trace" ? `Amazing ${playerName}! You traced the full knight's tour!` : `Great job, ${playerName}! Your answer has been saved.` },
    lose: { emoji: "😞", color: "#f87171", title: "Not quite right!", msg: gameMode === "trace" ? `You got stuck after ${tracedPath.length} moves. The full tour needs ${correctAnswer} moves!` : `The correct answer was ${correctAnswer} moves. Keep trying!` },
  ...
  // Get valid moves for highlighting in trace mode
  const currentValidMoves = gameMode === "trace" && tracedPath.length > 0
    ? getValidMoves(tracedPath[tracedPath.length - 1].row, tracedPath[tracedPath.length - 1].col)
    : [];
    ...
    <div ref={hubRef} style={{ background: "#050510", minHeight: "100vh", position: "relative", overflow: "hidden", fontFamily: "sans-serif" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, padding: "2rem 1.5rem" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2rem", flexWrap: "wrap", gap: "8px" }}>
          <button onClick={() => navigate("/")} style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", padding: "8px 16px", borderRadius: "20px", cursor: "pointer", fontSize: "13px" }}>← Back</button>
          <div style={{ display: "flex", gap: "8px" }}>
            <button onClick={fetchDashboard} disabled={dashboardLoading} style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", padding: "8px 16px", borderRadius: "20px", cursor: "pointer", fontSize: "13px" }}>
              {dashboardLoading ? "Loading..." : "📊 Dashboard"}
            </button>
            <button onClick={fetchLeaderboard} disabled={leaderboardLoading} style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", padding: "8px 16px", borderRadius: "20px", cursor: "pointer", fontSize: "13px" }}>
                    ...
                    {dashboardData.map((round, i) => (
                      <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "2px", flex: 1, minWidth: "40px" }}>
                        <div style={{ display: "flex", alignItems: "flex-end", gap: "2px", height: "180px" }}>
                          <div title={`Warnsdorff: ${round.algo1TimeMs}ms`} style={{ width: "14px", height: `${Math.max(4, (round.algo1TimeMs / maxTime) * 180)}px`, background: "#4ade80", borderRadius: "2px 2px 0 0", transition: "height 0.3s" }} />
                          <div title={`Backtracking: ${round.algo2TimeMs}ms`} style={{ width: "14px", height: `${Math.max(4, (round.algo2TimeMs / maxTime) * 180)}px`, background: "#60a5fa", borderRadius: "2px 2px 0 0", transition: "height 0.3s" }} />
                        </div>
              ...
              {dashboardData.length > 0 && (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginTop: "1.5rem" }}>
                  <div style={{ background: "rgba(74,222,128,0.1)", border: "1px solid rgba(74,222,128,0.2)", borderRadius: "10px", padding: "12px" }}>
                    ...
                    <p style={{ color: "#4ade80", fontSize: "20px", fontWeight: 600 }}>{Math.round(dashboardData.reduce((a, b) => a + (b.algo1TimeMs || 0), 0) / dashboardData.length)}ms</p>
                  ...
                  <div style={{ background: "rgba(96,165,250,0.1)", border: "1px solid rgba(96,165,250,0.2)", borderRadius: "10px", padding: "12px" }}>
                    ...
                    <p style={{ color: "#60a5fa", fontSize: "20px", fontWeight: 600 }}>{Math.round(dashboardData.filter(d => d.algo2TimeMs > 0).reduce((a, b) => a + (b.algo2TimeMs || 0), 0) / Math.max(1, dashboardData.filter(d => d.algo2TimeMs > 0).length))}ms</p>
            ...
            <div style={{ background: "#0a1628", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "16px", padding: "1.5rem", maxWidth: "600px", width: "100%", maxHeight: "95vh", overflowY: "auto" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
                <div>
                  ...
                  <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "12px", marginTop: "2px" }}>Move {vizStep + 1} of {solution.length}</p>
                </div>
                <button onClick={() => { setShowVisualization(false); setVizPlaying(false); }} style={{ background: "transparent", border: "none", color: "rgba(255,255,255,0.5)", fontSize: "20px", cursor: "pointer" }}>✕</button>
              </div>
              <div style={{ background: "rgba(55,138,221,0.15)", border: "1px solid rgba(55,138,221,0.3)", borderRadius: "10px", padding: "12px", marginBottom: "1rem", minHeight: "60px" }}>
                <p style={{ color: "rgba(255,255,255,0.9)", fontSize: "13px", lineHeight: "1.6" }}>{getVizExplanation(vizStep)}</p>
              </div>
                  ...
                  {Array.from({ length: boardSize }, (_, row) => (
                    <div key={row} style={{ display: "flex" }}>
                      {Array.from({ length: boardSize }, (_, col) => {
                        ...
                        if (row === startPos.row && col === startPos.col && vizStep === 0) bg = "#378ADD";
                        return (
                          <div key={col} style={{ width: vizCellSize, height: vizCellSize, background: bg, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", border: isCurrent ? "2px solid #fff" : "none", transition: "background 0.2s" }}>
                            {isCurrent && <span style={{ fontSize: boardSize === 8 ? "20px" : "12px" }}>♞</span>}
                            {isVisited && !isCurrent && <span style={{ fontSize: boardSize === 8 ? "10px" : "7px", color: "rgba(0,0,0,0.7)", fontWeight: 600 }}>{visitedIndex + 1}</span>}
                          </div>
                        );
                      })}
                    </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "1rem" }}>
                <button onClick={() => { setVizPlaying(false); setVizStep(0); }} style={{ padding: "8px 12px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.2)", background: "transparent", color: "#fff", cursor: "pointer", fontSize: "16px" }}>⏮</button>
                <button onClick={() => setVizStep(prev => Math.max(0, prev - 1))} disabled={vizStep === 0} style={{ padding: "8px 12px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.2)", background: "transparent", color: "#fff", cursor: "pointer", fontSize: "16px" }}>◀</button>
                <button onClick={() => setVizPlaying(p => !p)} style={{ flex: 1, padding: "8px 16px", borderRadius: "8px", border: "none", background: vizPlaying ? "#f87171" : "#4ade80", color: "#000", cursor: "pointer", fontSize: "14px", fontWeight: 600 }}>
                  {vizPlaying ? "⏸ Pause" : vizStep >= solution.length - 1 ? "↺ Replay" : "▶ Play"}
                </button>
                <button onClick={() => setVizStep(prev => Math.min(solution.length - 1, prev + 1))} disabled={vizStep >= solution.length - 1} style={{ padding: "8px 12px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.2)", background: "transparent", color: "#fff", cursor: "pointer", fontSize: "16px" }}>▶</button>
                <button onClick={() => { setVizPlaying(false); setVizStep(solution.length - 1); }} style={{ padding: "8px 12px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.2)", background: "transparent", color: "#fff", cursor: "pointer", fontSize: "16px" }}>⏭</button>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "1rem" }}>
                <span style={{ color: "rgba(255,255,255,0.5)", fontSize: "12px" }}>🐢 Slow</span>
                <input type="range" min="50" max="800" value={800 - vizSpeed + 50} onChange={e => setVizSpeed(800 - parseInt(e.target.value) + 50)} style={{ flex: 1, accentColor: "#378ADD" }} />
                <span style={{ color: "rgba(255,255,255,0.5)", fontSize: "12px" }}>⚡ Fast</span>
              </div>
              <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}><div style={{ width: "14px", height: "14px", background: "#facc15", borderRadius: "3px" }} /><span style={{ color: "rgba(255,255,255,0.5)", fontSize: "11px" }}>Current position</span></div>
                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}><div style={{ width: "14px", height: "14px", background: "#60a5fa", borderRadius: "3px" }} /><span style={{ color: "rgba(255,255,255,0.5)", fontSize: "11px" }}>Visited squares</span></div>
                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}><div style={{ width: "14px", height: "14px", background: "#f0d9b5", borderRadius: "3px" }} /><span style={{ color: "rgba(255,255,255,0.5)", fontSize: "11px" }}>Unvisited squares</span></div>
              </div>
            </div>
          </div>
        )}
        ...
        {/* ─── SETUP SCREEN ─── */}
        {gameState === "setup" && (
          <div style={{ maxWidth: "420px", margin: "0 auto", background: "rgba(13,59,110,0.6)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "16px", padding: "2rem" }}>
            ...
            {/* Game Mode Selection */}
            <div style={{ marginBottom: "1.5rem" }}>
              <label style={{ display: "block", color: "rgba(255,255,255,0.7)", fontSize: "13px", marginBottom: "8px" }}>Game mode</label>
              <div style={{ display: "flex", gap: "10px" }}>
                <button onClick={() => setGameMode("quick")}
                  style={{ flex: 1, padding: "12px 8px", borderRadius: "8px", border: `1px solid ${gameMode === "quick" ? "#4ade80" : "rgba(255,255,255,0.2)"}`, background: gameMode === "quick" ? "rgba(74,222,128,0.15)" : "rgba(255,255,255,0.05)", color: "#fff", cursor: "pointer", fontSize: "13px", textAlign: "center" }}>
                  <div style={{ fontSize: "20px", marginBottom: "4px" }}>⚡</div>
                  <strong>Quick Answer</strong>
                  <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.5)", marginTop: "4px" }}>Type the number of moves</div>
                </button>
                <button onClick={() => setGameMode("trace")}
                  style={{ flex: 1, padding: "12px 8px", borderRadius: "8px", border: `1px solid ${gameMode === "trace" ? "#f59e0b" : "rgba(255,255,255,0.2)"}`, background: gameMode === "trace" ? "rgba(245,158,11,0.15)" : "rgba(255,255,255,0.05)", color: "#fff", cursor: "pointer", fontSize: "13px", textAlign: "center" }}>
                  <div style={{ fontSize: "20px", marginBottom: "4px" }}>🎯</div>
                  <strong>Trace the Path</strong>
                  <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.5)", marginTop: "4px" }}>Click squares on board</div>
                </button>
              </div>
            </div>
        ...
        {gameState === "playing" && (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "1.5rem" }}>
            ...
            <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
              <div style={{ background: gameMode === "quick" ? "rgba(74,222,128,0.15)" : "rgba(245,158,11,0.15)", border: `1px solid ${gameMode === "quick" ? "rgba(74,222,128,0.4)" : "rgba(245,158,11,0.4)"}`, borderRadius: "20px", padding: "6px 14px", color: "#fff", fontSize: "13px" }}>
                {gameMode === "quick" ? "⚡ Quick Answer Mode" : "🎯 Trace the Path Mode"}
              </div>
            </div>

            <div style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "12px", padding: "12px 24px", color: "rgba(255,255,255,0.8)", fontSize: "14px", textAlign: "center" }}>
              ...
              {gameMode === "trace" && (
                <div style={{ marginTop: "6px", fontSize: "12px", color: "rgba(255,255,255,0.5)" }}>
                  {tracedPath.length === 0 ? "Click the blue square to start!" : `Moves: ${tracedPath.length} / ${boardSize * boardSize}`}
                </div>
              )}
            </div>
              ...
              {Array.from({ length: boardSize }, (_, row) => (
                <div key={row} style={{ display: "flex" }}>
                  {Array.from({ length: boardSize }, (_, col) => {
                    ...
                    const isLight = (row + col) % 2 === 0;
                    const isTraced = tracedPath.some(p => p.row === row && p.col === col);
                    const traceIndex = tracedPath.findIndex(p => p.row === row && p.col === col);
                    const isCurrent = tracedPath.length > 0 && tracedPath[tracedPath.length - 1].row === row && tracedPath[tracedPath.length - 1].col === col;
                    const isValidNext = gameMode === "trace" && currentValidMoves.some(m => m.row === row && m.col === col);
                    const isWrong = wrongMove && wrongMove.row === row && wrongMove.col === col;

                    let bg = isLight ? "#f0d9b5" : "#b58863";
                    if (gameMode === "trace") {
                      if (isWrong) bg = "#ef4444";
                      else if (isCurrent) bg = "#facc15";
                      else if (isTraced) bg = "rgba(96,165,250,0.7)";
                      else if (isStart && tracedPath.length === 0) bg = "#378ADD";
                      else if (isValidNext) bg = isLight ? "rgba(74,222,128,0.4)" : "rgba(74,222,128,0.3)";
                    } else {
                      if (isStart) bg = "#378ADD";
                    }
                    ...
                    return (
                      <div key={col}
                        ...
                        style={{
                          ...
                          fontSize: "10px", fontWeight: 600, color: "rgba(0,0,0,0.7)",
                          cursor: gameMode === "trace" ? "pointer" : "default",
                          transition: "background 0.15s",
                          border: isValidNext ? "1px solid rgba(74,222,128,0.8)" : "none",
                          position: "relative",
                        }}>
                        {isCurrent && gameMode === "trace" && <span style={{ fontSize: boardSize === 8 ? "22px" : "13px" }}>♞</span>}
                        {isStart && gameMode !== "trace" && <span style={{ fontSize: boardSize === 8 ? "22px" : "13px" }}>♞</span>}
                        {isTraced && !isCurrent && gameMode === "trace" && (
                          <span style={{ fontSize: boardSize === 8 ? "11px" : "7px" }}>{traceIndex + 1}</span>
                        )}
                      </div>
                    );
                  })}
            ...
            {/* Color legend for trace mode */}
            {gameMode === "trace" && (
              <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", justifyContent: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}><div style={{ width: "12px", height: "12px", background: "#facc15", borderRadius: "2px" }} /><span style={{ color: "rgba(255,255,255,0.5)", fontSize: "11px" }}>Current</span></div>
                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}><div style={{ width: "12px", height: "12px", background: "rgba(74,222,128,0.5)", borderRadius: "2px" }} /><span style={{ color: "rgba(255,255,255,0.5)", fontSize: "11px" }}>Valid moves</span></div>
                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}><div style={{ width: "12px", height: "12px", background: "rgba(96,165,250,0.7)", borderRadius: "2px" }} /><span style={{ color: "rgba(255,255,255,0.5)", fontSize: "11px" }}>Visited</span></div>
              </div>
            )}
            ...
            {gameMode === "trace" && (
              <button onClick={giveUp} disabled={loading}
                style={{ padding: "12px 24px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.2)", background: "transparent", color: "rgba(255,255,255,0.7)", fontSize: "14px", cursor: "pointer" }}>
                {loading ? "..." : "Give Up 🤝"}
              </button>
            )}
          </div>
        ...
        {gameState === "result" && result && (
          <div style={{ maxWidth: "450px", margin: "0 auto", textAlign: "center" }}>
            ...
            <h2 style={{ fontSize: "24px", fontWeight: 500, color: resultConfig[result].color, marginBottom: "8px" }}>{resultConfig[result].title}</h2>
            <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "14px", marginBottom: "1.5rem" }}>{resultConfig[result].msg}</p>
            ...
            <div style={{ background: "rgba(255,255,255,0.07)", borderRadius: "12px", padding: "1rem", marginBottom: "1rem", textAlign: "left" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <span style={{ color: "#60a5fa", fontSize: "13px", fontWeight: 500 }}>{boardSize === 16 ? "N/A (16×16)" : `${algo2Time}ms`}</span>
              </div>
              {boardSize === 8 && <p style={{ color: "rgba(255,255,255,0.3)", fontSize: "11px", marginTop: "8px" }}>* Backtracking has a 5 second timeout due to O(8^n) complexity</p>}
            </div>
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              <button onClick={resetGame} style={{ flex: 1, padding: "12px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.2)", background: "transparent", color: "#fff", fontSize: "14px", cursor: "pointer" }}>Play Again</button>
              <button onClick={fetchLeaderboard} style={{ flex: 1, padding: "12px", borderRadius: "8px", border: "1px solid rgba(55,138,221,0.5)", background: "rgba(55,138,221,0.2)", color: "#fff", fontSize: "14px", cursor: "pointer" }}>🏅 Leaderboard</button>
              <button onClick={() => navigate("/")} style={{ flex: 1, padding: "12px", borderRadius: "8px", border: "none", background: "#185FA5", color: "#fff", fontSize: "14px", cursor: "pointer" }}>Back to Hub</button>
            </div>
          </div>
        )}
      </div>
